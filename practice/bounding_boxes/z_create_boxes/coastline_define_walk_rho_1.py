# walking the mask grid, starting in lower right corner, assuming we start on land
# point, assuming the prevoius point was blow, and checking in a
# clockwise manner

# ... wouldn't this make more sense to do with the rho mask?
...
coastline_file_out = 'coastline_coords_rho_file_wc15_continent.p'
#---------------------------------------------------------------------
#---------------------------------------------------------------------
...
ii = 0
jj = n_j - 1
current_mask_point = mask[ii,jj]
while current_mask_point == 0:
    jj -= 1;
    current_mask_point = mask[ii,jj]
...
while True:
    ...
    if cp[1] < lp[1]:
        try:
            # look down
            if mask[ii-1,jj] < 1 and 0 <= ii-1 < n_i:
                ii -= 1
            # look left
            elif mask[ii,jj-1] < 1 and 0 <= jj-1 < n_j:
                jj -= 1
            # look up
            elif mask[ii+1,jj] < 1 and 0 <= ii+1 < n_i:
                ii += 1
            # look right
            elif mask[ii,jj+1] < 1 and 0 <= jj+1 < n_j:
                jj += 1
    ...
    # cp above lp
        ...
        try:
            # look left
            if mask[ii,jj-1] < 1 and 0 <= jj-1 < n_j:
                jj -= 1
            # look up
            elif mask[ii+1,jj] < 1 and 0 <= ii+1 < n_i:
                ii += 1
