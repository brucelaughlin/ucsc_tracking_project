#v3_minimal_test: Just want a few points, for swimming testing
...
points_in_boxes_file_out_continent = 'z_output/' + 'points_in_boxes_lon_lat_continent_swim_test.p'
points_in_boxes_file_out_islands = 'z_output/' + 'points_in_boxes_lon_lat_islands_swim_test.p'

points_in_boxes_file_out_combined = 'z_output/' + 'points_in_boxes_lon_lat_combined_swim_test.p'
#points_in_boxes_file_out_combined = 'z_output/' + 'points_in_boxes_lon_lat_combined_backup.p'
points_in_boxes_file_out_combined_ij = 'z_output/' + 'points_in_boxes_i_j_combined_swim_test.p'
...
for ii in range(n_i):
    for jj in range(n_j):
        ...
        points_i_j.append((ii,jj))


points_lon_lat = np.array(points_lon_lat)
points_i_j = np.array(points_i_j)
...
#for ii in range(3,len(boxes_lonlat)-3):
    ...
    if box_lonlat is not None:
        ...
        points_inside_ij = points_i_j[points_inside_flags]
        points_box_i = []
        points_box_j = []
        for point in points_inside_ij:
            points_box_i.append(point[0])
            points_box_j.append(point[1])
            break   #TESTING
        
        points_box_i_j = np.array([points_box_i,points_box_j])
        points_in_boxes_ij_continent.append(points_box_i_j)
...
points_in_boxes_lonlat_islands = []
points_in_boxes_ij_islands = []
...
for island_dex in range(num_last_blob_island,num_islands+1):
    ...
    for box_lonlat in boxes_lonlat:
        if box_lonlat is not None:
            ...
            points_in_boxes_lonlat_islands.append(points_box_lon_lat)
            points_in_boxes_ij_islands.append(points_box_i_j)
...
file = open(points_in_boxes_file_out_islands,'wb')
pickle.dump(points_in_boxes_lonlat_islands,file)
file.close()


# Store all seed points in a single list, for use in making a seed file

points_in_boxes_lonlat_combined = points_in_boxes_lonlat_continent + points_in_boxes_lonlat_islands
points_in_boxes_ij_combined = points_in_boxes_ij_continent + points_in_boxes_ij_islands

file = open(points_in_boxes_file_out_combined,'wb')
pickle.dump(points_in_boxes_lonlat_combined,file)
file.close()

file = open(points_in_boxes_file_out_combined_ij,'wb')
pickle.dump(points_in_boxes_ij_combined,file)
file.close()
