...
for jj in range(len(oxygen_limit_list)):
    pdf_list_exposure_oxygen = []
    for ii in range(5):
        pdf_list_exposure_oxygen.append(np.zeros((n_boxes,timesteps_full_run+1)))
        #pdf_list_exposure_oxygen.append(np.zeros((n_boxes,timesteps_full_run)))
        #pdf_list_exposure_oxygen.append(np.zeros((n_boxes,last_settlement_day)))
        #pdf_list_exposure_oxygen.append(np.zeros((n_boxes,timesteps_settlement_window+1)))
    pdf_list_of_lists_O2.append(pdf_list_exposure_oxygen)

pdf_list_of_lists_pH = []
for jj in range(len(pH_limit_list)):
    pdf_list_exposure_pH = []
    for ii in range(5):
        pdf_list_exposure_pH.append(np.zeros((n_boxes,timesteps_full_run+1)))
        #pdf_list_exposure_pH.append(np.zeros((n_boxes,timesteps_full_run)))
        #pdf_list_exposure_pH.append(np.zeros((n_boxes,last_settlement_day)))
        #pdf_list_exposure_pH.append(np.zeros((n_boxes,timesteps_settlement_window+1)))
    pdf_list_of_lists_pH.append(pdf_list_exposure_pH)
...
for ii in range(5):
    pdf_list_exposure_T.append(np.zeros((n_boxes,n_T_steps)))
...
#        continue
    ...
    drift_oxygen = np.zeros((num_particles,timesteps_full_run))
    ...
    for particle_id in range(len(particle_labels)):
        ...
        drift_oxygen[particle_id,:] = particle_oxygen[0:last_settle_dex]
        drift_pH[particle_id,:] = particle_pH[0:last_settle_dex]
        drift_T[particle_id,:] = particle_T[0:last_settle_dex]
        #drift_oxygen[particle_id,:] = particle_oxygen[1:last_settle_dex]
    ...
    particle_list_of_lists_pH = []
    for ii in range(len(pH_limit_list)):
        particle_list_of_lists_pH.append(np.zeros(num_particles))
    ...
    #for time_dex in range(timesteps_settlement_window):
        ...
        # Exposure 
        for ii in range(len(oxygen_limit_list)):
            particle_list_of_lists_O2[ii] = particle_list_of_lists_O2[ii] + ((current_oxygen < oxygen_limit_list[ii]).astype(int) * settlement_safety_mask)
            #particle_list_of_lists_O2[ii] = particle_list_of_lists_O2[ii] + ((current_oxygen < oxygen_limit_list[ii] + 0) * settlement_safety_mask)
        for ii in range(len(pH_limit_list)):
            particle_list_of_lists_pH[ii] = particle_list_of_lists_pH[ii] + ((current_pH < pH_limit_list[ii]).astype(int) * settlement_safety_mask)
            #particle_list_of_lists_pH[ii] = particle_list_of_lists_pH[ii] + ((current_pH < pH_limit_list[ii] + 0) * settlement_safety_mask)
        ...
        if time_dex >= first_settle_dex:
            print("first settle time dex: {}".format(time_dex))
        #------------------------------------------------------
        #------------------------------------------------------
        #------------------------------------------------------
    ...
    for ii in range(num_particles):
        ...
        if int(initial_boxes[ii]) == 0 or int(settlement_boxes[ii]) == 0:
            ...
            # Exposure 
            pdf_list_exposure_T[0][int(initial_boxes[ii])-1,average_T[ii]] += 1   
            for jj in range(len(oxygen_limit_list)):
                pdf_list_of_lists_O2[jj][0][int(initial_boxes[ii])-1,int(particle_list_of_lists_O2[jj][ii])] += 1   
            for jj in range(len(pH_limit_list)):
                pdf_list_of_lists_pH[jj][0][int(initial_boxes[ii])-1,int(particle_list_of_lists_pH[jj][ii])] += 1   
            ...
            if seed_months[ii] >=3 and seed_months[ii] <=5 : 
                pdf_list_exposure_T[2][int(initial_boxes[ii])-1,average_T[ii]] += 1   
                for jj in range(len(oxygen_limit_list)):
                    pdf_list_of_lists_O2[jj][2][int(initial_boxes[ii])-1,int(particle_list_of_lists_O2[jj][ii])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_list_of_lists_pH[jj][2][int(initial_boxes[ii])-1,int(particle_list_of_lists_pH[jj][ii])] += 1   
            ...
            #JJA
                ...
                pdf_list_exposure_T[3][int(initial_boxes[ii])-1,average_T[ii]] += 1   
                for jj in range(len(oxygen_limit_list)):
                    pdf_list_of_lists_O2[jj][3][int(initial_boxes[ii])-1,int(particle_list_of_lists_O2[jj][ii])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_list_of_lists_pH[jj][3][int(initial_boxes[ii])-1,int(particle_list_of_lists_pH[jj][ii])] += 1   
            ...
            #SON
                ...
                pdf_list_exposure_T[4][int(initial_boxes[ii])-1,average_T[ii]] += 1   
                for jj in range(len(oxygen_limit_list)):
                    pdf_list_of_lists_O2[jj][4][int(initial_boxes[ii])-1,int(particle_list_of_lists_O2[jj][ii])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_list_of_lists_pH[jj][4][int(initial_boxes[ii])-1,int(particle_list_of_lists_pH[jj][ii])] += 1   
            ...
            #DJF
                ...
                pdf_list_exposure_T[1][int(initial_boxes[ii])-1,average_T[ii]] += 1   
                for jj in range(len(oxygen_limit_list)):
                    pdf_list_of_lists_O2[jj][1][int(initial_boxes[ii])-1,int(particle_list_of_lists_O2[jj][ii])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_list_of_lists_pH[jj][1][int(initial_boxes[ii])-1,int(particle_list_of_lists_pH[jj][ii])] += 1   
...
d['pdf_list_exposure_T'] = pdf_list_exposure_T
#d['pdf_list_exposure_T'] = pdf_list_exposure_T
d['pdf_list_of_lists_O2'] = pdf_list_of_lists_O2
d['pdf_list_of_lists_pH'] = pdf_list_of_lists_pH
d['pdf_list_connectivity'] = pdf_list_connectivity
d['pdf_list_settleTime'] = pdf_list_settleTime
#d['pdf_list_settleTime'] = pdf_list_settleTime
...
#pickle.dump([bio_window_opening_distribution,pdf_list_exposure_T,pdf_list_of_lists_O2,pdf_list_connectivity,pdf_list_settleTime,counter_array,oxygen_limit_list],file)
#file.close()
