# v10: add pH along trajectories

# v9: ok so we don't want distributions of settlement eligibility.  so that work was a waste.  but, still want to switch to np.save instead of pickle
...
save_output_file_name = "pdf_data_output_seasonal_ranges_O2_pH___pld_{}_{}_".format(first_settlement_day-1,run_length_days-1) + tracking_output_dir_pre[0:-1]
...
for jj in range(len(pH_limit_list)):
    pdf_list_exposure_pH_source = []
    for ii in range(5):
        pdf_list_exposure_pH_source.append(np.zeros((n_boxes,timesteps_settlement_window+1)))
    pdf_list_of_lists_pH.append(pdf_list_exposure_pH_source)
