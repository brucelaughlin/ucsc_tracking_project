# v5: process longer drift times, with swim
...
# Hack - horizontal swimming made things weird, so trying some debugging stuff
dtCalc_minutes = 60
dtCalc = dtCalc_minutes * 60
...
#tracking_output_dir_pre = "drift_150_swim_speed_50/"
tracking_output_dir_pre = "drift_150_swim_speed_10/"
#---------------------------------------------------------------------

save_output_file_name = "pdf_data_output_seasonal_rangeO2_v5_" + tracking_output_dir_pre[0:-1] + ".p"
...
#---------------------------------------------------------------------
# PARAMETERS and CONSTANTS (actually constant)
...
## This breaks now that horizontal swimming is activated.  Is this because all particles reach the coast and deactivate?  But, they should bounce...???
## Either way, my first debug effort will be to hardcode the constants calculated here and see if things work
...
#if timesteps_per_day%1 != 0: sys.exit('(run timesteps)/(run_length_days) was not an integer! ({}/{})'.format(trajectory_mask.sum(),run_length_days))
...
timesteps_per_day = 86400/dtCalc
settlement_window_start = int(timesteps_per_day * first_settlement_day)
total_number_timesteps = int(timesteps_per_day * (run_length_days -1))
#total_number_timesteps = int(timesteps_per_day * (run_length_days))
...
for tracking_output_file_pre in tracking_output_files:
    ...
    for particle_id in range(len(particle_labels)):
        ...
        trajectory_mask = trajectory_status == 0
        if np.sum(trajectory_mask) > longest_drift:
            longest_drift = np.sum(trajectory_mask)
    print("Longest drift: {}".format(longest_drift))
    ...
    for time_dex in range(timesteps_settlement_window):
        ...
        #particle_list_exposure_oxygen = particle_list_exposure_oxygen + ((current_oxygen < oxygen_limit + 0) * settlement_safety_mask)
...
pickle.dump([pdf_list_exposure_T_source,pdf_list_of_lists_O2,pdf_list_connectivity,pdf_list_settleTime_source,settlement_boxes_test_array,settlement_times_test_array,counter_array,oxygen_limit_list],file)
#pickle.dump([pdf_list_exposure_T_source,pdf_list_of_lists_O2,pdf_list_connectivity,pdf_list_settleTime_source,settlement_boxes_test_array,settlement_times_test_array,counter_array],file)
#pickle.dump([pdf_list_exposure_T_source,pdf_list_exposure_oxygen_source,pdf_list_connectivity,pdf_list_settleTime_source,settlement_boxes_test_array,settlement_times_test_array,counter_array],file)
#pickle.dump([pdf_list_connectivity,pdf_list_settleTime_source,settlement_boxes_test_array,settlement_times_test_array,counter_array],file)
file.close()
