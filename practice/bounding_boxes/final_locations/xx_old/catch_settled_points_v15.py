...
# v15: Same problem as before, with other accumulating arrays - initializing with zeros creates problems when I want to track zero as a value.
#      So, need to do something like I've done elsewhere - add 1, then subract 1 later...

# v14: something is weird with the pH statistics - and I see that I reversed my intended row/column order for relevant arrays
...
pH_limit_list = [0,1,7.5,7.6,7.7,7.8,7.9,8,8.1,8.2,8.3,10]
        ...
        if int(initial_boxes[ii]) == 0 or int(settlement_boxes[ii]) == 0:
            ...
            for jj in range(len(O2_limit_list)):
                if particle_arrays_O2[ii,jj]) > 0:
                    pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                #pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])] += 1   
            for jj in range(len(pH_limit_list)):
                pdf_arrays_pH[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])-1] += 1   
                #pdf_arrays_pH[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])] += 1   
            pdf_arrays_connectivity[0,int(initial_boxes[ii])-1,int(settlement_boxes[ii])-1] += 1   
            pdf_arrays_settleTime[0,int(initial_boxes[ii])-1,int(settlement_times[ii])-1] += 1   
            ...
            if seed_months[ii] >=3 and seed_months[ii] <=5 : 
                ...
                for jj in range(len(O2_limit_list)):
                    if particle_arrays_O2[ii,jj]) > 0:
                        pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,2,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,2,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_arrays_pH[jj,2,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])-1] += 1   
                    #pdf_arrays_pH[jj,2,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])] += 1   
                pdf_arrays_connectivity[2,int(initial_boxes[ii])-1,int(settlement_boxes[ii])-1] += 1   
                pdf_arrays_settleTime[2,int(initial_boxes[ii])-1,int(settlement_times[ii])-1] += 1   
            ...
            #JJA
                ...
                for jj in range(len(O2_limit_list)):
                    if particle_arrays_O2[ii,jj]) > 0:
                        pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,3,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,3,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_arrays_pH[jj,3,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])-1] += 1   
                    #pdf_arrays_pH[jj,3,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])] += 1   
                pdf_arrays_connectivity[3,int(initial_boxes[ii])-1,int(settlement_boxes[ii])-1] += 1   
                pdf_arrays_settleTime[3,int(initial_boxes[ii])-1,int(settlement_times[ii])-1] += 1   
            ...
            #SON
                ...
                for jj in range(len(O2_limit_list)):
                    if particle_arrays_O2[ii,jj]) > 0:
                        pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,4,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,4,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_arrays_pH[jj,4,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])-1] += 1   
                    #pdf_arrays_pH[jj,4,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])] += 1   
                pdf_arrays_connectivity[4,int(initial_boxes[ii])-1,int(settlement_boxes[ii])-1] += 1   
                pdf_arrays_settleTime[4,int(initial_boxes[ii])-1,int(settlement_times[ii])-1] += 1   
            ...
            #DJF
                ...
                for jj in range(len(O2_limit_list)):
                    if particle_arrays_O2[ii,jj]) > 0:
                        pdf_arrays_O2[jj,0,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,1,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])-1] += 1   
                    #pdf_arrays_O2[jj,1,int(initial_boxes[ii])-1,int(particle_arrays_O2[ii,jj])] += 1   
                for jj in range(len(pH_limit_list)):
                    pdf_arrays_pH[jj,1,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])-1] += 1   
                    #pdf_arrays_pH[jj,1,int(initial_boxes[ii])-1,int(particle_arrays_pH[ii,jj])] += 1   
                pdf_arrays_connectivity[1,int(initial_boxes[ii])-1,int(settlement_boxes[ii])-1] += 1   
                pdf_arrays_settleTime[1,int(initial_boxes[ii])-1,int(settlement_times[ii])-1] += 1   
