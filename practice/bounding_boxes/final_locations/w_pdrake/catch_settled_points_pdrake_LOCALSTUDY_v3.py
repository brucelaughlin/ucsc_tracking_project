...
for island_dex in range(num_last_blob_island,num_islands+1):
        ...
        for box_lonlat in boxes_lonlat:
            num_boxes += 1
        #num_boxes += np.max([len(boxes_lonlat),len(boxes_lonlat[0])])
...
#    timesteps_full_run = (last_settlement_day+1) * timesteps_per_day + 1 # why isn't this just taken from the dimensions of the files?
    ...
    release_counts_per_cell = np.zeros((n_pdfs,num_boxes))


    # Connectivity (release box number vs settlement box number)
    pdf_arrays_connectivity = np.zeros((n_pdfs,num_boxes,num_boxes))

    # Time after PLD until settlement (saving only release location) (release box number vs settlement time)
    pdf_arrays_settleTime = np.zeros((n_pdfs,num_boxes,timesteps_settlement_window))
    ...
    #pdf_arrays_O2 = np.zeros((len(O2_limit_list),n_pdfs,num_boxes,timesteps_full_run))

    # Same idea for pH
    #pdf_arrays_pH = np.zeros((len(pH_limit_list),n_pdfs,num_boxes,timesteps_full_run))

    # Histogram of average temperature experienced - just estimating range, since I don't know it without processing
    #pdf_arrays_T = np.zeros((n_pdfs,num_boxes,n_T_steps))
        



    # Must track the "fake" local settlers, ie particles that never move!!!
    bogus_settlers_per_box = np.zeros(num_boxes)
                            ...
                            if settler_box_numbers[ii] != 0:
                                if settler_box_numbers[ii] == release_boxes[ii]:
                                    ...
                                    if particles_lon_all[ii,0] == particles_lon_all[ii,time_dex]:
                                        bogus_settlers_per_box[box_dex - 1] += 1
    ...
    #MUST FIX TIME AS WELL, AND THAT CAN'T BE HANDLED OUTSIDE OF THE LOOP LIKE THIS!!!!
    for ii in range(np.shape(pdf_arrays_connectivity)[1]):
        pdf_arrays_connectivity[array_dex,ii,ii] -= int(bogus_settlers_per_box[ii])

    print('Number of bogus settlers: {}'.format(sum(bogus_settlers_per_box)))
