...
pdf_plot = np.log10(np.transpose(pdf_raw))
...
mesh1 = axs.pcolormesh(X,Y,pdf_separated,cmap='jet',vmin=pdf_min_val,vmax=pdf_max_val)

plt.colorbar(mesh1)

plt.title("TESTING PDF of settlement vs seed location\n(should be only one dot)")
