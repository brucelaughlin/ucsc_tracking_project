# v2: Idea: add NaN mask where there are zeros in the pdfs - so that in the cdf, we have an idea of where there wasn't any data..

# v1: copied from v8 of the pdf plotting, and using plot_pdf_swapped_1D_times_cdf_v1.py as a template for the cdf stuff
...
fig_mainTitle = "CDFs of average temperature experienced (y-axis) vs release box number (x-axis).\nGrouped according to season of release."
...
for pdf in pdf_arrays_T[1:]:
    ...
    cdf_mask = pdf_full == 0
    cdf_full = np.zeros_like(pdf_full)

    for ii in range(np.shape(cdf_full)[1]):
        cdf_full[:,ii] = np.sum(pdf_full[:,0:ii+1],axis=1)
    
    if np.amax(cdf_full) > cdf_max_val:
        cdf_max_val = np.amax(cdf_full)
    if np.amin(np.ma.masked_invalid(cdf_full)) < cdf_min_val:
        cdf_min_val = np.amin(np.ma.masked_invalid(cdf_full))
    
    cdf_full[cdf_mask] = np.nan
    cdf_list_full.append(cdf_full)



num_dummy_lines = 1

n_rows = int(np.shape(cdf_list_full[0])[0]) + num_dummy_lines
n_columns = int(np.shape(cdf_list_full[0])[1])
...
x_line_positions = []
for ii in range(np.shape(cdf_list_full[0])[0]):
...
for ii in range(len(cdf_list_full)):
    ...
    if ii == 0:
    #if ii == 1:
        mesh1 = axs[0,0].pcolormesh(X,Y,cdf_separated.T,cmap=cmap_use,vmin=cdf_min_val,vmax=cdf_max_val)
...
#        axs[0,0].set_xticklabels(tick_labels_double_X, fontsize=label_fontSize)
    ...
    #elif ii == 2:
        mesh1 = axs[0,1].pcolormesh(X,Y,cdf_separated.T,cmap=cmap_use,vmin=cdf_min_val,vmax=cdf_max_val)
    ...
    #elif ii == 3:
        mesh1 = axs[1,0].pcolormesh(X,Y,cdf_separated.T,cmap=cmap_use,vmin=cdf_min_val,vmax=cdf_max_val)
        #mesh1 = axs[1,0].pcolormesh(X,Y,cdf_separated.T,cmap='jet',vmin=cdf_min_val,vmax=cdf_max_val)
        ...
        mesh1 = axs[1,1].pcolormesh(X,Y,cdf_separated.T,cmap=cmap_use,vmin=cdf_min_val,vmax=cdf_max_val)
        #mesh1 = axs[1,1].pcolormesh(X,Y,cdf_separated.T,cmap='jet',vmin=cdf_min_val,vmax=cdf_max_val)
