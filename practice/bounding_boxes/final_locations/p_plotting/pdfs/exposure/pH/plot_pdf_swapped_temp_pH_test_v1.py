...
#pH_limit_list = [7.6,7.7,7.8]  # REMOVE THIS AFTER RUNNING CATCH SCRIPT AGAIN
test_dex = 0
#------------------------------------------------------------------
...
#test_dex = 3
# -------------------------------------------------------------------
pdf_list_exposure_pH_source = pdf_list_of_lists_pH[test_dex]
# -------------------------------------------------------------------
# -------------------------------------------------------------------
...
fig_main_title_pre_p1 = "PDFs of cumulative number of days settled larvae were exposed to pH concentrations below "
fig_main_title_pre_p2 = r'$\bf{{{a}}}$'.format(a=pH_limit_list[test_dex])
fig_main_title_pre_p3 = " mg/L (y-axis) vs Release Location (x-axis).\nGrouped according to season of release.\n(\"0 day\" probabilities plotted below main plot as a line plot)"
...
y_label = y_label_pre.format(pH_limit_list[test_dex])
# -------------------------------------------------------------------
...
pdf_min_val = 999999
# -------------------------------------------------------------------
for pdf in pdf_list_exposure_pH_source[1:]:
    pdf_full = pdf.copy()
    # -------------------------------------------------------------------
...
n_rows = int(np.shape(pdf_list_exposure_pH_source[0])[0]) + num_dummy_lines
n_columns = int(np.shape(pdf_list_exposure_pH_source[0])[1])
...
day_0_title = day_0_title_pre.format(pH_limit_list[test_dex])
day_1_60_title_pre2 = day_1_60_title_pre.format(pH_limit_list[test_dex])
...
#------------------------------------------------------------------
# These tick number determining values are dependent on which pH level we're measuring
#------------------------------------------------------------------
if test_dex < 3:
...
for ii in range(len(pdf_list_full)):
    ...
    if ii == 0:
        ...
        axs[1,0].set_yticklabels(y_ticks_day1,fontsize=label_fontSize)
