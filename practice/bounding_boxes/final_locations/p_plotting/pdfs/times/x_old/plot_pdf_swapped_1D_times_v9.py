...
# V9: calculate basic stats - 10%/90% settle by...

# V8: i'm dumb - with log, don't need to separate the 1st huge bin
...
stats_vectors_hist = np.zeros((len(pdf_list_settleTime)-1,np.shape(pdf_list_settleTime[0])[1]))
stats_vectors = np.zeros((len(pdf_list_settleTime)-1,np.shape(pdf_list_settleTime[0])[1]))
