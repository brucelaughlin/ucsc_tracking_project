...
#tick_positions_orig = [1,5,8,10,13,16,17,19,23,29,32,37,41,47,56,60,67,78]
                ...
                if switch_plot_islands:
                    #if box_num % 3 == 0:
                    ax.annotate(box_num, xy = [np.mean(box[0]), np.mean(box[1])], color=number_color)
                box_num += 1
        ...
        if tick_num < len(tick_labels):
            if box_num == tick_positions[tick_num]:
                if box_num in [30,33]:
                    ...
                    ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                        #xytext =(.2 * offset, .1 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color, va=va_val)
                        #xytext =(-.9 * offset, 0), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops2, color=text_color, va=va_val)
                        #xytext =(0, .3 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops3, color=text_color)
