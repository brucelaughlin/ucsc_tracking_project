...
#tick_positions_orig = [1,5,8,10,13,16,17,19,23,29,32,37,41,47,56,60,67,78]
                    if box_num in [2,11,14]:
                        ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                            xytext =(-.6 * offset, -.2 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color)
                    elif box_num in [6]:
                        ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                            xytext =(.4 * offset, -.25 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color)
                    elif box_num in [17]:
                        ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                            xytext =(-.5 * offset, -.4 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color)
                    else:
                        ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                            xytext =(-.6 * offset, .2 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color)
        ...
        if tick_num < len(tick_labels):
            if box_num == tick_positions[tick_num]:
                if box_num in [33,38]:
                    ...
                    ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                        xytext =(.2 * offset, .1 * offset), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops, color=text_color)
                else:
                    ax.annotate("{}: {}".format(box_num,tick_labels[tick_num]), xy = [np.mean(box[0]), np.mean(box[1])],
                        xytext =(.3 * offset, 0), textcoords ='offset points', bbox = bbox, arrowprops = arrowprops2, color=text_color)
                tick_num += 1
