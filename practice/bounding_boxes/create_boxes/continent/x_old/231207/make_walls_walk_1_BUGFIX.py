...
while walk_switch == True:
    ...
    for jj in range(coast_dex,num_points_coast):
        ...
        polygon_test_i = polygon_i[:] + list(isoline_i[closest_dex:isoline_dex-1:-1])
        polygon_test_j = polygon_j[:] + list(isoline_j[closest_dex:isoline_dex-1:-1])
        ...
        if closest_dex == num_points_isoline-1 or jj == num_points_coast-1:
            ...
            walk_switch = False
            print(str(PolyArea(polygon_test_i,polygon_test_j)))
            break

        if PolyArea(polygon_test_i,polygon_test_j) >= box_area_threshold:
