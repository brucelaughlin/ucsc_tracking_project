...
for island_number in range(1,num_islands_intersecting+1):   
    ...
    if island_number < num_islands_intersecting :
        coastline_file_in_next = input_dir + 'coastline_coords_wc15n_island_number_{}.p'.format(island_number+1)
        file = open(coastline_file_in_next,'rb')
        coastline_lonlat_next = pickle.load(file)
        file.close
        coastline_lonlat_next = coastline_lonlat_next[0]
    ...
    if island_number < num_islands_intersecting :
        coast_lon_north.append((coastline_lonlat[bridge_point_list[island_number - 1][1],0] + coastline_lonlat_next[bridge_point_list[island_number][0],0])/2.0)
