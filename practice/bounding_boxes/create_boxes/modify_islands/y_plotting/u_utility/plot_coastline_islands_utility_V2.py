...
#X = np.arange(-.5,int(np.shape(lon_field)[0]-1),1)
#Y = np.arange(-.5,int(np.shape(lon_field)[1]-1),1)
#X = np.arange(-.5,int(np.shape(lon_field)[0]),1)
#Y = np.arange(-.5,int(np.shape(lon_field)[1]),1)
#print(np.shape(mask))
X = range(np.shape(mask)[1])
Y = range(np.shape(mask)[0])
...
#for island_dex in range(num_island,num_island+1):   
    ...
    for coastline in coastlines_lonlat:
        if coastline is not None:
            ...
            ax.scatter(coastline[:,0],coastline[:,1],c='b') 
            ax.scatter(coastline[0,0],coastline[0,1],c='r') 
            for ii in range(np.shape(coastline)[0]):
                ...
                #ax.annotate(box_dex, xy = [coastline[ii,0],coastline[ii,1]], color='white', ha="center", va="center", fontsize=15, weight="bold")
                box_dex += 1
