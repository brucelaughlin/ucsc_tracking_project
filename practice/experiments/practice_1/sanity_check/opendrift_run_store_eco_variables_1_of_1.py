...
# Sanity Check: run for just one box, make sure that the starting depths are correct (ie make sure
# I didn't accidentally swap i,j when referencing h).  Also look at plots, which should hopefully
# be easy to make with a smaller set of floats.

# Turns out that I should have recalled what I already knew.  Seeding just once used up about 15% of the head
# node memory.... So I can probably only have a maximum of 5 seedings of floats running on a node at one time.

# Old metadata - number of months seeding, number of total runs to cover the total months, total seeding months
#test_metadata = '18_01_18'

# New metadata: number of days seeding (semi-daily), number of total runs to cover the total seeding days, total seeding days
test_metadata = '01_01_01'
...
n_days_seed = int(metadata_split[0])
#n_runs = int(int(metadata_split[2])/n_months_seed)
n_runs = int(int(metadata_split[2])/n_days_seed)
...
# Now set a fixed box number - perhaps.... 20?
sanity_box_number = 20

for run_day in range(0,n_days_seed,2):
    #for ii in range(len(points_in_boxes_lon_lat)):
    for ii in range (sanity_box_number,sanity_box_number + 1):
        # Iterate over all box entries, where boxes have dimensions 2x(num_points)
