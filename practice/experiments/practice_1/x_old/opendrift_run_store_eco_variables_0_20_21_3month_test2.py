...
# Trying to add readers from list, so theyre added lazily, which isn't going to work because
# I'm relying on a custom reader.  Awesome.
...
while year_val <= end_year:
    ...
    his_dir = history_base + his_dir_year

    his_year_list = os.listdir(his_dir)
    his_year_list.sort()

    his_year_list = [his_dir + s for s in his_year_list]

    o.add_readers_from_list(his_year_list)
