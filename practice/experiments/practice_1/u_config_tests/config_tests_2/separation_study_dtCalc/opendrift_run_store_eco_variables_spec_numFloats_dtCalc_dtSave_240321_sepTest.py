...
#for run_day in range(0,end_seed_time,2): #this is broken, end_seed_time is not int.  also should not start at 0, but rather "start_seed_time" right?
#for run_day in range(int(start_seed_time),int(end_seed_time),2):  # something like this?  what do these ints look like? 
    for ii in range(len(points_in_boxes_lon_lat)):
        for jj in range(np.shape(points_in_boxes_lon_lat[ii])[1]):
        #for jj in range(0,np.shape(points_in_boxes_lon_lat[ii])[1],20):
        #for jj in range(1):
