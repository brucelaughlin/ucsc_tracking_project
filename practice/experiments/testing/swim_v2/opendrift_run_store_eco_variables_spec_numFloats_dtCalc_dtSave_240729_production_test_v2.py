...
# v2: try seeding at all grid points.  Something is wrong with the gradient file, while the constant swim file seems to work.
...
#lons = lon_rho.flatten()
#lats = lat_rho.flatten()
zs = list(np.zeros_like(lons))
times = list(np.full(np.shape(lons),datetime.datetime.strptime(str(start_seed_time+datetime.timedelta(days=0)), '%Y-%m-%d %H:%M:%S')))
