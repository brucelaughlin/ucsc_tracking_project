...
#while [ ! -f z_output_2/log_${logString}.txt ]; do
    sleep 20
done
...
#while [[ $(tail -n 1 z_output_2/log_${logString}.txt | awk '{print $NF}') != Finished ]]; do
    if [ ! -f z_output/log_${logString}.txt ]; then
    #if [ ! -f z_output_2/log_${logString}.txt ]; then
        break
    fi
    sleep 10 
    ...
    #echo ${arr[1]:0:-1} >> z_output_2/run_memory_info_${logString}.txt
done
