...
for ii in ${startNudgeArray[@]}; do
    ...
    #python opendrift_run_store_eco_variables_spec_numFloats_dtCalc_dtSave_240413_production.py $dtCalc $dtSave $bufferLength $parentDir $nSeed $ii $readerDir1 $readerDir2 &> z_output/z_logs/log_${logString}.txt &
    python opendrift_run_store_eco_variables_spec_numFloats_dtCalc_dtSave_240413_production.py $dtCalc $dtSave $bufferLength $outputDir $nSeed $ii $readerDir1 $readerDir2 &> z_output/z_logs/log_${logString}.txt &
