...
for filename in file_name_list:
    ...
    buffer_length_pre1 = filename.split('.txt')[0]
    buffer_length = int(buffer_length_pre1.split('_')[-1])

    buffer_length_list.append(buffer_length)
...
slope = (runtime_list[-1]-runtime_list[0])/(buffer_length_list[-1]-buffer_length_list[0])

fig, ax = plt.subplots()
ax.plot(buffer_length_list[:],runtime_list[:],marker='*', label='run time')
ax.plot(buffer_length_list[:],exectime_list[:],marker='*', label='execution time')
...
for i_x, i_y in zip(buffer_length_list,runtime_list):
